...
SEQ = [
    ...
    ('DIV-1b_to-act-two',       'clips/DIV-P.mp4'),
    ...
    ('DIV-1c_to-lasers',        'clips/DIV-R.mp4'),
    ...
    ('DIV-2_to-fence',          'clips/DIV-S.mp4'),
    ...
    ('DIV-3_to-night',          'clips/DIV-P.mp4'),
    ...
    ('DIV-4_to-morning',        'clips/DIV-S.mp4'),
    # act four - collapse
...
TRIM = {
    'CO-2_carrier':                     0.26,   # re-shot; line at 0.86s -> shot 5.78s
    ...
    'A-6_name-one':                     2.50,   # line at 3.10s -> shot 1.54s
    ...
    'B-5_the-drawer':                   2.54,   # line at 3.14s -> shot 1.50s
    'B-6_no-laser-drawer':              2.09,   # line at 2.69s -> shot 2.95s
    'B-8_who-here-is-tired':            2.01,   # line at 3.71s -> shot 5.09s
    'B-10_nobodys-told-you':            1.54,   # re-shot; line at 2.14s -> shot 2.50s
    ...
    'C-7_i-own-a-bowl':                 2.55,   # cuts the stray "Mooney," the model added;
                                                # real line at 2.75s -> shot 1.49s
    ...
    'E-1_i-wanted-to-help':             1.80,   # re-shot; line at 2.40s -> shot 5.24s
    'E-2_i-know':                       3.62,   # line at 4.22s -> shot 2.42s
    'E-4_thats-not':                    3.08,   # re-shot; line at 3.68s -> shot 2.96s
    'E-5_id-do-it-again':               3.30,   # line at 3.90s -> shot 2.74s
}

# Tail trims. The re-shot shots of 2026-09-11 were capped to one line each
# ("He says EXACTLY these words and NOTHING else"), which removed the ad-libbed
# second utterance that used to fill the back half of the shot - and exposed a
# long silent hold in its place. Cut so roughly 0.8s of air follows the line.
TAIL = {
    'A-8_and':              2.44,   # sound ends 0.74s into a 4.04s shot
    'A-11_garys-fine':      1.14,   # sound ends 2.08s
    'D-4_you-had-a-bowl':   2.34,   # sound ends 1.82s
}
...
def main():
    ...
    for i, (name, path) in enumerate(SEQ):
        ...
        t += d
        ins += (['-ss', '%.3f' % t0] if t0 else [])
        ins += (['-t', '%.3f' % d] if t1 else []) + ['-i', path]
        parts.append(
